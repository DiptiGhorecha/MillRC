...
''' <summary>
''' Tables used - bill_tr,bill,godown,part,rent
''' when any closed suspended godown reopen user need to generate invoice for multiple months
''' this module is used for it. When user select invoice date, first it checks that bulk invoices generated or not. Allow to proceed only if bulk invoice generated.
''' When user select group and godown number, get invoice number in serial itself and fill all other form details.  user is allowed to add from date and to date. When user click update button invoices for selected months are generated
...
Public Class FrmInvMultiple
    ...
    Private Sub LodaDataToTextBox()
        ...
        Try
            Dim i As Integer
            TextBox1.Text = ""    ''''godown number - invisible 
            ...
            If DateTime.Now.Month = 1 Then     '''''set invoice date as last day of the month
            ...
            If DataGridView2.RowCount > 0 Then
                ...
                If Not IsDBNull(DataGridView2.Item(4, i).Value) Then
                    DateTimePicker1.Value = GetValue(DataGridView2.Item(4, i).Value)        '''''bill_date from bill_tr table
                    DateTimePicker2.Value = GetValue(DataGridView2.Item(15, i).Value)       '''''from_date from bill_tr table
                    DateTimePicker3.Value = GetValue(DataGridView2.Item(16, i).Value)       '''''''to_date from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(1, i).Value) Then
                    ComboBox3.Text = GetValue(DataGridView2.Item(1, i).Value)               ''''''group from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(2, i).Value) Then
                    TextBox1.Text = GetValue(DataGridView2.Item(2, i).Value)                '''''''godwn_no from bill_tr table
                    ComboBox4.Text = GetValue(DataGridView2.Item(2, i).Value)               '''''''godwn_no from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(0, i).Value) Then
                    TextBox2.Text = GetValue(DataGridView2.Item(0, i).Value)                ''''invoice_no from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(11, i).Value) Then
                    TextBox3.Text = GetValue(DataGridView2.Item(11, i).Value)              '''''hsn from bill_tr table
                ...
                If Not IsDBNull(DataGridView2.Item(5, i).Value) Then
                    TextBox4.Text = Format(CSng(GetValue(DataGridView2.Item(5, i).Value) / (DateDiff(DateInterval.Month, DateTimePicker2.Value, DateTimePicker3.Value) + 1)), "#####0.00")   ''''''set monthly bill amount , bill_amt from bill_tr table
                    TextBox5.Text = Format(CSng(GetValue(DataGridView2.Item(5, i).Value)), "#####0.00")
                End If
                If Not IsDBNull(DataGridView2.Item(7, i).Value) Then
                    TextBox6.Text = Format(CSng(GetValue(DataGridView2.Item(7, i).Value)), "#####0.00")      ''''''cgst_amt from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(9, i).Value) Then
                    TextBox7.Text = Format(CSng(GetValue(DataGridView2.Item(9, i).Value)), "#####0.00")       ''''''''sgst_amt from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(10, i).Value) Then
                    TextBox8.Text = Format(CSng(GetValue(DataGridView2.Item(10, i).Value)), "#####0.00")      '''''''net_amount from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(6, i).Value) Then
                    TextBox9.Text = GetValue(DataGridView2.Item(6, i).Value)             '''''''cgst_rate from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(8, i).Value) Then
                    TextBox10.Text = GetValue(DataGridView2.Item(8, i).Value)            ''''''sgst_rate from bill_tr table
                End If
                If Not IsDBNull(DataGridView2.Item(12, i).Value) Then
                    TextBox13.Text = DataGridView2.Item(12, i).Value                    '''''srno from bill_tr table
                End If
    ...
    Function print_display_data()
        ...
        Do While chkrs.EOF = False
            ...
            doSQL(save)    '''''insert invoice data in bill table
            doSQL(savetr)   ''''''insert invoice data in bill_tr table
            FileClose(fnum)
    ...
    Private Sub cmdUpdate_Click(sender As Object, e As EventArgs) Handles cmdUpdate.Click
        ...
        If ValidateChildren() Then
            ...
            If (Not System.IO.Directory.Exists(Application.StartupPath & "\Invoices\dat\" & DateTimePicker1.Value.Year.ToString & "\" & MonthName(DateTimePicker1.Value.Month))) Then
                System.IO.Directory.CreateDirectory(Application.StartupPath & "\Invoices\dat\" & DateTimePicker1.Value.Year.ToString & "\" & MonthName(DateTimePicker1.Value.Month))
            End If
            ''''''''generated invoice .dat file
            strReportFilePath = Application.StartupPath & "\Invoices\dat\" & DateTimePicker1.Value.Year.ToString & "\" & MonthName(DateTimePicker1.Value.Month) & "\" & FILE_NO & ".dat"
            CreatePDF(strReportFilePath, FILE_NO)    '''' convert .dat file to pdf file
    ...
    Private Sub doSQL(ByVal sql As String)
        '''''''method to store data in database table
        MyConn = New OleDbConnection(connString)
        If MyConn.State = ConnectionState.Closed Then
            MyConn.Open()
        End If
    ...
    Private Sub DateTimePicker1_ValueChanged(sender As Object, e As EventArgs) Handles DateTimePicker1.ValueChanged
        '''''value of date time picker will be last day of the month
    ...
    Private Sub ComboBox4_TextUpdate(sender As Object, e As EventArgs) Handles ComboBox4.TextUpdate
        ''''''to select item when user type any letter on combo box
