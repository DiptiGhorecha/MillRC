...
Public Class Form16
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            xcon.Open("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" & Application.StartupPath & "\millrc.accdb;")
        End If
        Dim str As String = "SELECT [RECEIPT].* from [RECEIPT] where [RECEIPT].REC_DATE between FORMAT('" & strtP & "','DD/MM/YYYY') AND FORMAT('" & edP & "','DD/MM/YYYY') order by [RECEIPT].REC_DATE,[RECEIPT].REC_NO"
        ...
        Do While chkrs1.EOF = False
            ...
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            Dim tr As String = "SELECT [GODOWN].*,[PARTY].P_NAME from [GODOWN] INNER JOIN [PARTY] on [GODOWN].P_CODE=[PARTY].P_CODE WHERE [GROUP]='" & chkrs1.Fields(1).Value & "' AND GODWN_NO='" & chkrs1.Fields(2).Value & "' AND [STATUS]='C'"
            ...
            advanceamt = chkrs1.Fields(5).Value - adjusted_amt
                ...
                If chkrs1.Fields(6).Value = True And residential = False Then
                    ...
                    tot3 = tot3 + ((advanceamtprint * 100 / 118) * 9 / 100)
