...
''' <summary>
''' tables used - bill,godown,party,rent,receipt,reciptbill
''' using this module we can bulk generate invoice for selected date
''' check that advance is received for that year-month, if yes mention it in invoice
''' </summary>
Public Class FrmInvoice
    ...
    Private Function checkpr()
        Try
            ...
            ''''''''check in bill table for selected month and year if bills are already generated
            ...
            '''''''deciding financial year from invoice date selected
            Dim iDate As String
            ...
            xpage = Val("2")
            Dim i1 As Double
            Dim numRec As Integer = 0      ''''''''variable to store number of invoices generated

            ''''''Loop through all godown records which are active for selected invoice date 
            ...
            Do While chkrs.EOF = False
                numRec = numRec + 1
                srno = srno + 1
                Dim INVOICE_NO As String    '''''''variable to store invoice number
                Dim FILE_NO As String   ''''''''variable used to store pdf filename for invoice
                Dim FILE_NOtmp As String '''''''variable ued to store refrence number
                ...
                ''''''''directory created to store invoices is in format \invoices\dat\<year>\<month>, check if directory exist, if not create it
                ...
                ''''open empty dat file to write invoice text
                ...
                Dim TENANT_CODE As String   ''''partycode
                Dim TENANT_NAME As String   ''''partyname
                Dim T_ADDREASS As String    ''''address line1
                Dim TAD1 As String          ''''address line2
                Dim TAD2 As String          ''''address line3
                Dim TAD3 As String          ''''address line4
                Dim TCITY As String         ''''city
                Dim TSTATE As String        ''''state 
                Dim STATE_CODE As String    ''''state code
                Dim TEMAIL As String        ''''party email address
                Dim TGST As String          ''''party gst number
                Dim amt As Double           ''''invoice amount
                Dim CGST_TAXAMT As Double   ''''cgst amount
                Dim SGST_TAXAMT As Double   ''''sgst amount
                Dim CGST_RATE As Double     ''''cgst rate 9%
                Dim gst_amt As Double       '''' gst amount (cgst amount+sgst amount)
                Dim SGST_RATE As Double     '''' sgst rate
                Dim gst As Double           '''' gst
                ...
                ''''''select latest rent detail for current party and godown number
                ...
                If chkrs2.EOF = False Then
                    ...
                    amt = chkrs2.Fields(4).Value   '''''added rent to amount
                    If IsDBNull(chkrs2.Fields(5).Value) Then
                    Else
                        amt = amt + chkrs2.Fields(5).Value  ''''if any permitted increase value, add it to amount
                    End If
                End If
                ...
                '''''''select gst details from gst table - for gst field in godown table
                ...
                Dim net As Double
                Dim rnd As Integer
                rnd = gst_amt - Math.Round(gst_amt)      ''''''calculate rounded off amount
                If rnd >= 50 Then
                    gst_amt = Math.Round(gst_amt) + 1    '''''''if rounded off amount more than 50 paise add 1 rupees
                Else
                ...
                ''''''for cgst amount devide it by 2 and again round off
                CGST_TAXAMT = Math.Round(gst_amt / 2, 2, MidpointRounding.AwayFromZero)
                ''''''for sgst amount devide it by 2 and again round off
                ...
                inwordd = convinRS(net)    '''''''get words for net amount
                ...
                ''''take receipts with advance amount
                ...
                If chkrs2.EOF = False Then
                    ...
                    adjamt = chkrs2.Fields(13).Value - net  ''''''reduce invoice amount from advance amount to be adjusted

                    Dim sav As String = "UPDATE [RECEIPT] SET ADJ_AMT=" & adjamt & " WHERE REC_NO=" & chkrs2.Fields(4).Value & " AND REC_DATE=format('" & Convert.ToDateTime(chkrs2.Fields(3).Value) & "','dd/mm/yyyy')"
                    doSQL(sav)
                End If
                chkrs2.Close()
                ...
                Dim save As String
                If advrec > 0 Then    '''''''if advance received than insert data in bill table along with receipt number, receipt date and advance
                ...
                Else                  '''''''insert data in bill table without receipt number, receipt date and advance
                ...
                ''''''''directory created to store invoices pdf in format \invoices\dat\<year>\<month>, check if directory exist, if not create it
                ...
                ''''''''''''''''dat file used to convert into pdf file
                ...
                CreatePDF(strReportFilePath, FILE_NO)     '''''''create pdf file for invoice
    ...
    Private Sub doSQL(ByVal sql As String)
        '''''Method to insert/update data in database tables
    ...
    Private Function CreatePDF(strReportFilePath As String, invoice_no As String)
        ''''''methos for crating pdf 
        Try
            Dim line As String
            Dim readFile As System.IO.TextReader = New StreamReader(strReportFilePath)   ''''''''.dat file
            Dim yPoint As Integer = 0

            Dim pdf As PdfDocument = New PdfDocument
            pdf.Info.Title = "Text File to PDF"
            Dim pdfPage As PdfPage = pdf.AddPage
            pdfPage.TrimMargins.Left = 15   '''' for left margin

            Dim graph As XGraphics = XGraphics.FromPdfPage(pdfPage)


            Dim font As XFont = New XFont("COURIER NEW", 9, XFontStyle.Regular)   ''''''set font size and type
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        With Me
            ...
            .Refresh()
        End With
        checkpr()   '''''' main function for invoice generate
        With Me
    ...
    Private Sub DateTimePicker1_ValueChanged(sender As Object, e As EventArgs) Handles DateTimePicker1.ValueChanged
        ''''''''set datepicker value - last day of the month for selected date
    ...
    Private Sub FrmInvoice_Move(sender As Object, e As EventArgs) Handles Me.Move
        ''''' to keep the position of the form fix on MDI form
        If formloaded Then
